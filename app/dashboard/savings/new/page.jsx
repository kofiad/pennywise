...
export default function NewSavings({initialData={}, isUpdate=false}) {
  ...
  async function onSubmit(data){
    console.log(data);
    if (isUpdate) {
      makePutRequest(setloading, `api/savings/${initialData.id}`,redirect, data);
    } else {
      makePostRequest(setloading, "api/savings", data, reset);
    }
  };
  ...
  return (
    <div>
        {/* Header */}
        <FormHeader title={isUpdate?"Update savings":"New savings"} href="/dashboard/savings"/>
        {/* Form */}
        <form onSubmit={handleSubmit(onSubmit)} className='w-full max-w-4xl mx-auto p-4 my-3 bg-purple-100 border border-gray-200 rounded-lg shadow sm:p-6 md:p-8 dark:bg-gray-800 dark:border-gray-700'>
          <div className='w-full grid gap-4 grid-cols-1 sm:gap-6'>
            ...
            <div className="flex items-center justify-center w-full"><SubmitButton isLoading={loading} title= {isUpdate?"Update":"savings"}/></div>
          </div>
        </form>
    </div>
  )
